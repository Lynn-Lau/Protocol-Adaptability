...
We propose an evaluation model on the basis of communication efficiency, then measure the ANP by calculating
the communication efficiency of the new protocols evolves to the final states through the model. We also
...
can be accepted by users after the protocols are applied to the network [7][8]. The degree of users' acceptance
...
degree of users' acceptance. That makes different for lowering the costs and reducing the blindness of protocol
...
visited 19 experts and have a discussion about the problems of HIP protocol is not widely used and its deployment
...
%  \captionsetup{font={scriptsize}}
 % \caption{The Connection Relationship of ASPNs}
 % \begin{tabular}{|l|c|c|c|c|}
  %\hline
  %\diagbox{ASPN Nodes}{UN Nodes} & $A$ & $AB$ & $B$ & $BA$ \\
  %\hline
  %$A$ & ASPN A user with state A \\
   %   %& ASPN B user with state BA
    %  & ASPN B users with state AB \\
     % %& ASPN B user with state B
      %& $\times$ & $\times$ \\
  %\hline
  %$B$ & $\times$ & $\times$
   %                 & ASPN B user with state B \\
    %                & ASPN A user with state AB
     %               & ASPN user with state A \\
      %              & ASPN user with state BA \\
  %\hline
  %\end{tabular}
%\end{table}
